...
# 5) XGBOOST IMPORTANCES ON FULL DATA → top5 & top10
xgb_full = XGBClassifier(n_estimators=100, random_state=42)
xgb_full.fit(X, y)
xgb_imp_full = pd.Series(xgb_full.feature_importances_, index=X.columns)\
                   .sort_values(ascending=False)
top5_xgb_full    = xgb_imp_full.index[:5].tolist()
top10_xgb_full   = xgb_imp_full.index[:10].tolist()

print("XGBoost – top 5 features on FULL data:")
print(top5_xgb_full)

# 6) XGBOOST IMPORTANCES ON REDUCED (TOP-10) DATA → top5
xgb_reduced = XGBClassifier(n_estimators=100, random_state=42)
xgb_reduced.fit(X[top10_xgb_full], y)
xgb_imp_reduced = pd.Series(xgb_reduced.feature_importances_, index=top10_xgb_full)\
                      .sort_values(ascending=False)
top5_xgb_reduced = xgb_imp_reduced.index[:5].tolist()

print("\nXGBoost – top 5 features on REDUCED (top-10) data:")
print(top5_xgb_reduced)
...
print("\nXGB_full_top5 importances:")
print(xgb_imp_full[top5_xgb_full])

print("\nXGB_reduced_top5 importances:")
print(xgb_imp_reduced[top5_xgb_reduced])
...
cases = [
    ("XGB_full_top5",    top5_xgb_full),
    ("XGB_reduced_top5", top5_xgb_reduced),
...
for name, feats in cases:
    scores = cross_val_score(xgb, X[feats], y, cv=cv, scoring='accuracy')
